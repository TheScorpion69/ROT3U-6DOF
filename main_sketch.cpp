...
class SGServo {
  ...
  // 544 - 2400 мкс ???
  // Управляющий сигнал представляет из себя импульсы с нужной нам шириной, которые посылаются с определенной частотой.
  // Для рассматриваемых нами сервоприводов частота посылания импульса почти всегда будет около 50 Гц (это примерно 1 раз в 20мс),
  // а ширина импульса будет лежать в пределе от 544мкс до 2400мкс.
